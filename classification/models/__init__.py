...
def build_vssm_model(config, is_pretrain=False):
    ...
    if model_type in ["vssm"]:
        model = VSSM(
            ...
            # ===================
            top_k=config.MODEL.VSSM.TOP_K,
            knn=config.MODEL.VSSM.KNN,
            alpha=config.MODEL.VSSM.ALPHA,
            ambiguity=config.MODEL.VSSM.AMBIGUITY,
            binary=config.MODEL.VSSM.BINARY,
            k_group=config.MODEL.VSSM.K_GROUP,
            division_rate=config.MODEL.VSSM.DIVISION_RATE,
            mode=config.MODEL.VSSM.MODE,
            dimension=config.MODEL.VSSM.DIMENSION,
            csms6s_mode=config.MODEL.VSSM.CSMS6S_MODE,
            # ===================
